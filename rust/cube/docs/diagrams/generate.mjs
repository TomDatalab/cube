...
function edgePoint(b, tx, ty) {
  ...
  const sx = (b.w / 2) / Math.abs(dx || 1e-9); const sy = (b.h / 2) / Math.abs(dy || 1e-9);
...
function resolveArrow(a, boxes) {
  ...
  const fc = [f.x + f.w / 2, f.y + f.h / 2]; const tc = [t.x + t.w / 2, t.y + t.h / 2];
...
function textSvg(lines, x, y, { size = 20, color = C.ink, align = 'middle', weight } = {}) {
  ...
  return lines.map((l, i) => `<text x="${x}" y="${top + i * lh}" font-size="${size}" fill="${color}" text-anchor="${align}"` +
...
function toSvg(d) {
  ...
  const paths = (drawable) => gen.toPaths(drawable).map((p) => `<path d="${p.d}" stroke="${p.stroke}" stroke-width="${p.strokeWidth}" fill="${p.fill || 'none'}"` +
  ...
  for (const s of d.shapes.filter((a) => a.arrow || a.points)) {
    ...
    for (const [[x1, y1], [x2, y2]] of heads) {
      const ang = Math.atan2(y2 - y1, x2 - x1); const L = 14; const W = 0.45;
...
function toExcalidraw(d) {
  ...
  const text = (t, x, y, size, color, containerId, align = 'center') => {
    const lines = t.split('\n');
    const w = Math.max(...lines.map((l) => l.length)) * size * 0.55; const h = lines.length * size * 1.25;
    return base({
      id: id('text'),
      type: 'text',
      x: align === 'left' ? x : x - w / 2,
      y: y - h / 2,
      width: w,
      height: h,
      strokeColor: color,
  ...
  for (const s of d.shapes.filter((a) => a.arrow || a.points)) {
    ...
    const xs = rel.map((p) => p[0]); const ys = rel.map((p) => p[1]);
    const a = base({
      id: id('arrow'),
      type: 'arrow',
      x: x0,
  ...
  return `${JSON.stringify({
    type: 'excalidraw',
    version: 2,
    source: 'https://github.com/TomDatalab/cube',
    elements: els,
    appState: { viewBackgroundColor: C.white, gridSize: null },
    files: {},
  }, null, 2)}\n`;
}
...
// 1. Before and after -------------------------------------------------------
{
  const L = 40; const R = 760; const W = 620;
  const node = (id, y, text, stroke, fill, x = L + 30, w = W - 60, h = 58) => ({ box: id, x, y, w, h, text, stroke, fill });
...
// 3. Migration roadmap -----------------------------------------------------
{
  ...
  steps.forEach(([n, title, text], i) => {
    const x = 40 + i * 212; const done = i < 7;
    shapes.push({ box: `s${i}`,
      x,
      y: 110,
      w: 188,
      ...
      fill: done ? C.greenBg : C.orangeBg,
      size: 15 });
  ...
  shapes.push({ label: 'Each step: port the Node.js code and its tests, keep paths, bodies, status codes and CUBEJS_* names,\n' +
    'fail with a named error where something is not supported yet, never degrade silently.',
  x: 870,
  y: 360,
  size: 18,
  color: C.grey });
